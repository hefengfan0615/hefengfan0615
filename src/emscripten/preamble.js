// Preamble for the Pikafish WASM build.
// Exposes the engine to the web page / Node through Module.send_command():
//   - FS.init() routes the engine's stdin/stdout through JS callbacks so each
//     UCI line can be injected by send_command() and output captured via
//     Module.read_stdout().
//   - Module.send_command(data) feeds one UCI command into the engine (it is
//     processed synchronously by wasm_uci_execute()).
// Reference: https://github.com/official-pikafish/Pikafish (wasm branch).
Module["engine_ready"] = Module["engine_ready"] || false;
Module["send_command"] = Module["send_command"] || (() => {});
Module["read_stdout"] = Module["read_stdout"] || ((output) => console.log(output));
if (!Module["preRun"]) Module["preRun"] = [];
Module["preRun"].push(function () {
  let input = {
    str: "",
    index: 0,
    set: function (str) {
      this.str = str + "\n";
      this.index = 0;
    },
  };

  let output = {
    str: "",
    flush: function () {
      Module.read_stdout(this.str);
      this.str = "";
    },
  };

  function stdin() {
    // Return ASCII code of character, or null if no input
    let char = input.str.charCodeAt(input.index++);
    return isNaN(char) ? null : char;
  }

  function stdout(char) {
    if (!char || char == "\n".charCodeAt(0)) {
      output.flush();
    } else {
      output.str += String.fromCharCode(char);
    }
  }

  FS.init(stdin, stdout, stdout);
  let wasm_uci_execute = Module.cwrap("wasm_uci_execute", "void", []);
  Module.send_command = function (data) {
    input.set(data);
    wasm_uci_execute();
  };
});
Module["onRuntimeInitialized"] = function () {
  Module.engine_ready = true;
};
